...
namespace Microsoft.Iris.Input
{
    public class KeyboardDevice : InputDevice
    {
        ...
        public const int KeyChar = 2;
        public const int SysKeyDown = 3;
        public const int SysKeyUp = 4;
        public const int SysChar = 5;
        ...
        public KeyInfo OnRawKeyCharacter(uint message, ref RawKeyboardData args) => KeyCharacterInfo.Create(KeyAction.Character, args._deviceType, this.Manager.Modifiers, args._repCount, (char)args._virtualKey, message == 5U, message, args._scanCode, args._flags);

        public KeyInfo OnRawKeyState(
          uint message,
        ...
        public void MarkKeyHandled(Keys key)
        {
            foreach (KeyboardDevice.KeyState keyState in this._keyStates)
            {
                if (keyState.VKey == key && keyState.IsDown)
